...
local GetOverlayPanel = GetOverlayPanel
...
do
	...
	function AddLegacy( text, type, length )
		...
		local Panel = vgui_Create( "NoticePanel", parent )
		Panel.StartTime = SysTime()
...
local function UpdateNotice( pnl, total_h )
	...
	if ( math_abs( dist ) < 2 and math_abs( pnl.VelY ) < 0.1 ) then pnl.VelY = 0 end
	dist = ideal_x - x
	pnl.VelX = pnl.VelX + dist * spd * 1
	if ( math_abs( dist ) < 2 and math_abs( pnl.VelX ) < 0.1 ) then pnl.VelX = 0 end
...
do
	...
	function PANEL:Paint( w, h )
		...
		surface_SetDrawColor( 0, 100, 0, 150 )
		surface_DrawRect( boxX, boxY, boxW, boxH )

		surface_SetDrawColor( 0, 50, 0, 255 )
		surface_DrawRect( boxX + 1, boxY + 1, boxW - 2, boxH - 2 )

		local w = math_ceil( boxInnerW * 0.25 )
		local x = math_fmod( math_floor( SysTime() * 200 ), boxInnerW + w ) - w

		if ( self.ProgressFrac ) then
			x = 0
			w = math_ceil( boxInnerW * self.ProgressFrac )
		end

		if ( x + w > boxInnerW ) then w = math_ceil( boxInnerW - x ) end
		if ( x < 0 ) then
			w = w + x
			x = 0
		end

		surface_SetDrawColor( 0, 255, 0, 255 )
		surface_DrawRect( boxX + 1 + x, boxY + 1, w, boxH - 2 )
