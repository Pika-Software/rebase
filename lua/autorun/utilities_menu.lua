...
if CreateClientConVar("cl_multicore", "1", true, true, " - Garry's Mod multicore", 0, 1):GetBool() then
...
cvars.AddChangeCallback("cl_multicore", function( name, old, new )
	if (new == "1") then
		RunConsoleCommand("gmod_mcore_test", 1)
		RunConsoleCommand("mat_queue_mode", -1)
		RunConsoleCommand("cl_threaded_bone_setup", 1)
...
local function SandboxClientSettings( pnl )
	...
	AddCheckbox( "Multi-core", "cl_multicore" )
